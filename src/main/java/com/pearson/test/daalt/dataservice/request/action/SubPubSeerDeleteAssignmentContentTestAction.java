...
public class SubPubSeerDeleteAssignmentContentTestAction extends DeleteAssignmentContentTestAction {

	public SubPubSeerDeleteAssignmentContentTestAction(Instructor instructor, CourseSection courseSection, Assignment assignment) {
	...
	public void execute(int seerCount, int subPubCount) throws Exception {
		...
		System.out.println("Now executing DeleteAssignmentContentTestAction...\n");
		
		//Learning_Module message to create the Assignment
		Message revelLmCreateMessage = new LearningModuleMessage();
		revelLmCreateMessage.setProperty("Transaction_Type_Code", "Delete");
		...
		for (Chapter chapter : assignment.getChapters()) {
			...
			chapterMessage.setProperty("Transaction_Type_Code", "Delete");
			...
			if (chapterQuiz != null) {
				...
				chapterQuizMessage.setProperty("Transaction_Type_Code", "Delete");
				...
				if (chapterQuiz.getNestedQuizzes() != null){
					for (Quiz qz : chapterQuiz.getNestedQuizzes()) {
						...
						nestedQuizMessage.setProperty("Transaction_Type_Code", "Delete");
			...
			if (chapter.getChapterSections() != null){
				for (ChapterSection chapSection : chapter.getChapterSections()) {
					...
					chapterSectionMessage.setProperty("Transaction_Type_Code", "Delete");
					...
					if (chapterSectionQuiz != null) {
						...
						chapterSectionQuizMessage.setProperty("Transaction_Type_Code", "Delete");
					...
					if (chapSection.getPages() != null) {
						for (Page page : chapSection.getPages()) {
							...
							readingPageMessage.setProperty("Transaction_Type_Code", "Delete");
							...
							if (page.getPages() != null) {
								for (Page pg : page.getPages()){
									...
									readingPgMessage.setProperty("Transaction_Type_Code", "Delete");
							...
							if (page.getEmbeddedQuestions() != null) {
								...
								for (Question embeddedQuestion : page.getEmbeddedQuestions()) {
									...
									embeddedQuestionLMCMessage.setProperty("Transaction_Type_Code", "Delete");
