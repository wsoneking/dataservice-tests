...
/**
 * Assignment contains a chapter quiz with three types of multi-value questions: 
 * FillInTheBlank, Numeric, RadioButton. Two students enrolled in course section. 
 * 
 * Student_1 answers all questions correctly on first attempt. 
 * Student_2 uses two attempts on all questions, answers some correctly and some incorrectly.
 * 
 * The RadioButton question is “seeded” (via Assessment_Item_Possible_Answers message) 
 * with all possible answers listed and one marked as “Correct”. 
 * If the student answers this question correctly, 
 * the selected answer indicated in the Multi_Value_User_Answered_Question message 
 * will match the correct answer in the “seed” message.
 * 
 * For the FillInTheBlank and Numeric questions, the student can answer the question correctly 
 * even if the answer indicated in the Multi_Value_User_Answered_Question message 
 * does not match any of the possible answers in the “seed” (Assessment_Item_Possible_Answers) message. 
 * To test this, some of the “Correct” answers sent in the Multi_Value_User_Answered_Question message 
 * will not match answers in the “seed” message. 
 * 
 * Also, for the first question, student_1 will submit a different answer than student_2 and both will be marked as correct,
 * and for the second question, student_1 and student_2 will submit the same answer, 
 * but one is marked “Correct” and the other is marked “Incorrect”
 */
public class FillInTheBlankNumericRadioButtonQuestions extends BaseTestScenario {
	...
	public void loadBasicTestData() throws Exception {
		...
		try {
			...
			testData = new BasicTestData("FillInTheBlankNumericRadioButtonQuestions");
			...
			assignmentToAdd = assignmentFactory.getAssignmentByEnum(AssignmentFactory.AssignmentLibrary.CHAPTER_QUIZ_WITH_FILL_IN_THE_BLANK_AND_NUMERIC);
			...
			//student1 answers RadioButton_Question_01 correctly on the first attempt
			timeSpent = 17;
			Question chapterQuizQuestion04 = chapterQuiz.getQuestions().get(3);
			Answer question04CorrectAnswer = chapterQuizQuestion04.getSubQuestions().get(0).getCorrectAnswer().getAnswers().get(0);
			MultiValueAttempt stud01Question04Attempt01 = getMultiValueRadioButtonAttempt(chapterQuizQuestion04, student1, timeSpent, /*attemptNumber*/ 1, 
					question04CorrectAnswer, /*pointsEarned*/ chapterQuizQuestion04.getPointsPossible(), /*isFinalAttempt*/ true);
			chapterQuizQuestion04.addAttempt(stud01Question04Attempt01);
			timeOnTaskOutput.append("\n...assessment time : " + student1.getPersonId() + " : " + chapterQuizQuestion04.getId() + " : " + timeSpent + " seconds");
			QuestionCompletionActivity question04Stud01CompletionActivity = new QuestionCompletionActivity(student1, chapterQuizQuestion04.getPointsPossible());
			chapterQuizQuestion04.addCompletionActivity(question04Stud01CompletionActivity);
			TestAction stud01Question04Attempt01Action = testActionFactory.getAttemptQuestionTestAction(courseSection, assignmentToAdd, chapterQuiz, chapterQuizQuestion04, stud01Question04Attempt01, question04Stud01CompletionActivity);
			secondRequest.addTestAction(stud01Question04Attempt01Action);
			...
			stud1ChapQuizCompletionActivity.addQuestionPerf(chapterQuizQuestion04.getId(), chapterQuizQuestion04.getPointsPossible());
			...
			//TODO: student2 first attempt for RadioButton_Question_01 (incorrect)
			timeSpent = 10; 
			Answer chapSecQuizQues4IncorrectAnswer01 = chapterQuizQuestion04.getSubQuestions().get(0).getSingleIncorrectAnswer().getAnswers().get(0);
			MultiValueAttempt stud02Question04Attempt01 = getMultiValueRadioButtonAttempt(chapterQuizQuestion04, student2, /*timeSpent*/ timeSpent, /*attemptNumber*/ 1, chapSecQuizQues4IncorrectAnswer01, 0f, /*isFinalAttempt*/ false);
			chapterQuizQuestion04.addAttempt(stud02Question04Attempt01);
			timeOnTaskOutput.append("\n...assessment time : " + student2.getPersonId() + " : " + chapterQuizQuestion04.getId() + " : "+ timeSpent + " seconds");
			TestAction stud02Question04Attempt01Action = testActionFactory.getAttemptQuestionTestAction(courseSection, assignmentToAdd, chapterQuiz, chapterQuizQuestion04, stud02Question04Attempt01, null);
			thirdRequest.addTestAction(stud02Question04Attempt01Action);		
			
			//TODO: student2 second attempt for RadioButton_Question_01 (incorrect & final)
			timeSpent = 11; 
			MultiValueAttempt stud02Question04Attempt02 = getMultiValueRadioButtonAttempt(chapterQuizQuestion04, student2, /*timeSpent*/ timeSpent, /*attemptNumber*/ 2, chapSecQuizQues4IncorrectAnswer01, 0f, /*isFinalAttempt*/ true);
			chapterQuizQuestion04.addAttempt(stud02Question04Attempt02);
			timeOnTaskOutput.append("\n...assessment time : " + student2.getPersonId() + " : " + chapterQuizQuestion04.getId() + " : "+ timeSpent + " seconds");
			QuestionCompletionActivity question04Stud02CompletionActivity = new QuestionCompletionActivity(student2,/*score*/0);
			chapterQuizQuestion04.addCompletionActivity(question04Stud02CompletionActivity);
			TestAction stud02Question04Attempt02Action = testActionFactory.getAttemptQuestionTestAction(courseSection, assignmentToAdd, chapterQuiz, chapterQuizQuestion04, stud02Question04Attempt02, question04Stud02CompletionActivity);
			thirdRequest.addTestAction(stud02Question04Attempt02Action);
			...
			stud2ChapQuizCompletionActivity.addQuestionPerf(chapterQuizQuestion04.getId(),0);
