...
import com.pearson.test.daalt.dataservice.request.message.UserAnsweredItemWithUnknownFormat;

public class SubPubSeerAttemptUnknownFormatQuestionTestAction extends AttemptUnknownFormatQuestionTestAction {
	
	public SubPubSeerAttemptUnknownFormatQuestionTestAction(CourseSection courseSection,
	...
	public void execute(int seerCount, int subPubCount) throws Exception {
		...
		System.out.println("Now executing AttemptUnknownFormatQuestionTestAction...\n");
		
		boolean finalAttempt = attempt.isFinalAttempt() ? true : false;
		
		Message message = new UserAnsweredItemWithUnknownFormat(finalAttempt);
